...
<div class="max-w-4xl mx-auto bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden mb-10">
    ...
    <div class="bg-green-700 px-6 py-4 flex justify-between items-center rounded-t-lg">
        <h3 class="text-xl font-bold text-white flex items-center gap-2">
            <i class="fa-solid fa-bell-concierge"></i> Tambah Layanan Villa
        </h3>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.cabanglayanan-villa.store', $lokasi) }}" method="POST" class="p-6 bg-white border border-gray-200 rounded-b-lg shadow-sm">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Extra Bed, BBQ Set" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
            ...
            <div>
                ...
                <input type="number" name="price" value="{{ old('price') }}" placeholder="0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-green-500 focus:border-green-500" placeholder="Jelaskan detail layanan...">{{ old('description') }}</textarea>
            </div>
        </div>

        <div class="flex justify-end gap-3 mt-6">
            <button type="reset" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Reset</button>
            <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-green-700 rounded-lg hover:bg-green-800">Simpan Data</button>
        </div>
    </form>
</div>
@endsection
