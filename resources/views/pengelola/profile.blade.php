    ...
    <nav class="flex mb-4" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            <li class="inline-flex items-center">
                <a href="{{ route('pengelola.dashboard') }}" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-green-600 transition-colors">
...
<div class="w-full px-0 sm:px-2 md:px-4 lg:px-6 xl:px-10 mx-auto grid">
    ...
    {{-- 2. HEADER HALO --}}
    <div class="mb-8">
        <a href="{{ route('pengelola.dashboard') }}" class="inline-flex items-center gap-2 text-gray-500 mb-2 hover:text-green-600 text-sm transition-colors">
        <i class="fas fa-arrow-left"></i>
        <span>Kembali</span>
    </a>
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Halo, {{ $pengelola->nama_lengkap }} 👋🏻</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {{-- Sebelah Kiri --}}
        <div class="bg-gray-300 rounded-2xl p-6 h-fit shadow-sm">
            <div class="flex flex-col items-center mb-6">
                <div class="relative">
                    @if($pengelola->foto_profil)
                        <img id="img-preview" class="w-28 h-28 rounded-full object-cover border-4 border-white shadow-md transition-opacity group-hover:opacity-90"
                            src="{{ asset('storage/' . $pengelola->foto_profil) }}?t={{ time() }}"
                            alt="{{ $pengelola->nama_lengkap }}">
                        @error('foto_profil') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    @else
                        <img id="img-preview" class="w-28 h-28 rounded-full object-cover border-4 border-white shadow-md transition-opacity group-hover:opacity-90"
                            src="https://ui-avatars.com/api/?name={{ urlencode($pengelola->nama_lengkap) }}&background=random&size=128"
                            alt="Default Avatar">
                    @endif
                </div>
                <h2 class="mt-4 text-xl font-bold text-gray-900">{{ $pengelola->nama_lengkap }}</h2>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="text-sm font-medium text-gray-800 border-b border-gray-400 pb-1">{{ $pengelola->username }}</div>
                </div>
                <div>
                    <label class="block text-xs text-gray-500 ">Email:</label>
                    <div class="text-sm font-medium text-gray-800 border-b border-gray-400 pb-1">{{ $pengelola->email }}</div>
                </div>
                <div>
                    <label class="block text-xs text-gray-500 ">No Telepon:</label>
                    <div class="text-sm font-medium text-gray-800 border-b border-gray-400 pb-1">{{ $pengelola->no_telp ?? '-' }}</div>
                </div>
            </div>
        </div>
            ...
            <form action="{{ route('pengelola.profile.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $pengelola->nama_lengkap) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="username" value="{{ old('username', $pengelola->username) }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $pengelola->email) }}"
                            class="py-3 px-4 block w-full bg-gray-200 border-transparent rounded-lg text-sm focus:border-green-500 focus:ring-green-500 focus:bg-white transition-all disabled:opacity-50 disabled:pointer-events-none" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_telp" value="{{ old('no_telp', $pengelola->no_telp) }}"
