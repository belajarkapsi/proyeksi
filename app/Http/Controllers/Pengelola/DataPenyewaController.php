...
{
    private function idCabang()
    {
        return auth('pengelola')->user()->id_cabang;
    }

    public function index()
    {
        $users = User::whereHas('pemesanan', function($query) {
            $query->where('id_cabang', $this->idCabang());
        })
        ->withCount(['pemesanan as total_pesanan_cabang' => function($query) {
            $query->where('id_cabang', $this->idCabang());
        }])
        ->orderBy('nama_lengkap', 'asc')
        ->paginate(10);

        return view('pengelola.data-penyewa.index', compact('users'));
    }
    ...
    public function show(string $id)
    {
        ...
        $riwayatPemesanan = Pemesanan::where('id_penyewa', $id)
            ->where('id_cabang', $this->idCabang())
            ->orderBy('waktu_pemesanan', 'desc')
            ->get();

        return view('pengelola.data-penyewa.show', compact('user', 'riwayatPemesanan'));
    }
    ...
    public function edit(String $id)
    {
        ...
        return view('pengelola.data-penyewa.edit', compact('user'));
    }
    ...
    public function update(Request $request, String $id)
    {
        ...
        return redirect()->route('pengelola.penyewa.index');
    }
}
